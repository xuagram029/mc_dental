...
const regDentist = (req, res) => {
  ...
  db.query(
    "SELECT * FROM dentists WHERE username = ?",
    [username],
    (err, result) => {
      if (err) return res.sendStatus(500);
      ...
      db.query(
        "INSERT INTO dentists(`name`, `specialty`, `number`, `username`, `password`) VALUES (?)",
        [values],
        (err, data) => {
          if (err) {
...
const updateDentist = (req, res) => {
  ...
  db.query(
    "UPDATE dentists SET `name` = ?, `specialty` = ?, `number` = ? WHERE id = ?",
    [name, specialty, number, id],
    (err, result) => {
      if (err) {
